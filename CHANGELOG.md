## 2026-09-14 - release cascade restored; codesearch literal mode finally live

**plugkit-bin had been frozen at v0.1.1284 since 2026-09-10.** Every main push since then failed in the reusable release workflow's sign step because the `RELEASE_SIGNING_KEY` secret is not configured on this repo, so the literal/regex codesearch fast path (`8137c76`) and the git-commit-vector sync cap (`52a74db`) never reached a consumer. A daemon on v0.1.1284 does not know `mode: "literal"` and silently serves `dual`, which on a real 2.4k-file repo (C:/dev/freddie) meant a digest-mismatch rebuild plus an unbounded commit-vector embed pass: 88-365s per call, one witnessed at 371s against a 120s MCP timeout. `agentplug-runner` verifies the sha256 sidecar only and never reads a minisig, so the sign step now warns and continues when the key is absent (and still signs when it is present). v0.1.1288 published from the first push after the fix; the same literal query now answers in 880ms warm (2369 files listed, `.gm/`, `node_modules/`, `.git/` never walked, `exhaustive: true`).

**Native `cargo test` compiles again.** `residual_scan_marker_matches_current_session_or_is_within_longgap_threshold` in `orchestrator/transitions.rs` called `crate::wasm_dispatch::host_now_ms` without the `wasm32` cfg gate that everything around it carries, so the non-wasm lib target (the one `cargo test` builds) failed with E0433 -- the release CI only builds the wasm guest, which is why it went unnoticed.

## 2026-09-14 - code_index: whole-tree symbol coverage on large monorepos

**Fixed the codeinsight index covering almost none of a large tree.** Live-witnessed on a 3300-file pnpm monorepo (1474 JS/TS, 1308 markdown): every `instruction` reported `file_count: 4..80`, `by_kind` all `section`, digest `files=92:partial=406`, and `codesearch` for a JS symbol returned only markdown hits. Four compounding causes, all in `code_index.rs`: (1) every auto-index caller passed a hardcoded 500-file limit while `current_digest()` walked 2000, so the index could never cover the tree and the digest could never match it; (2) the walk indexed files in directory order, which put the whole `docs/` tree ahead of `packages/`; (3) the per-pass chunk cap (`remaining_ms / 16000`, typically 1-2 chunks per file) truncated a file to its first chunk or two and then wrote a manifest that every later pass reused as complete, so the truncation was permanent; (4) call edges were stored one kv row per edge and `delete_call_edges_for_path` re-read the whole namespace per indexed file -- measured ~0.5s per file, O(files x edges), which alone consumed the entire wall budget.

**Changes:** the index limit is `IndexConfig::digest_max_files` (default raised 2000 -> 20000, `prune_pass_file_limit_ceiling` likewise) for every auto-index caller and the `codeinsight_index` verb default; files are sorted source-code first, markdown second, data/config third before the limit is applied; extraction is decoupled from embedding -- the file loop extracts symbols for every reachable file and persists chunks whose vector is not yet known with an empty `emb` (manifest v7; BM25 and symbol lookup serve them immediately), then an embed loop spends the remaining budget on pending chunks in file order, and a file stays `pending`/deferred (digest keeps `:partial=N`) until every chunk has a vector; call edges are one kv row per file, deleted by key, with the legacy per-edge rows purged once. Witnessed: one pass now extracts 1868 files / 18664 chunks (was 92 files / 96 chunks after many passes) and `codesearch {"query":"overview"}` returns `OverviewSection` from `packages/client/ui-trajectory/src/client/TrajectoryTable.js`.

**`overview()` no longer reports `counts_error: unknown_plugin` as if the index were empty.** Each pass writes an index summary to the host kv store (`codeinsight-summary` namespace); the overview always carries `extracted_files`/`extracted_symbols`/`pending_embed_files`/`pending_embed_chunks` from it, and when the libsql sibling plugin is absent or evicted the counts fall back to that summary with `counts_source: "index_summary"` and `chunk_store_unavailable: <code>` naming the real failure.
