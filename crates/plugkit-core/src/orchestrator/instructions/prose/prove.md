...
**Search-only-via-verb binds mid-PROVE hardest.** Every code/file/symbol/filename lookup -- every ad-hoc where-is-this / what-calls-that / find-the-definition / locate-by-name -- is a `codesearch` dispatch (`mode:"filename"` for locate-by-name/glob), full stop. Never a platform Explore agent, raw `Grep`/`Glob`/`find`, or a "quick" cat/read used as discovery. Mid-PROVE lookups are not exempt as "just checking something": the orienting surface at SPECIFY is the SAME surface mid-PROVE, no downgrade to raw tools because you are already inside the phase. Exempt only: `Read` on an already-known specific path.
