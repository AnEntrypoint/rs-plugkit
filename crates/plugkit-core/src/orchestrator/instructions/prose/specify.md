...
**Search-only-via-verb, hard rule.** `codesearch`/`recall` are the ONLY code/file/symbol/filename discovery surfaces at SPECIFY -- `codesearch` body `{"query":"...","mode":"filename"}` covers locate-by-name/glob, the same job a `find`/Glob dispatch would otherwise be reached for. Raw `Read`/`Glob`/`Grep`/`find` used AS exploration/discovery (open-ended "where is X", "what calls Y", tree-walk, locate-by-name) is a deviation -- same class as reaching for puppeteer over the `browser` verb. Exempt: `Read` on a SPECIFIC already-located path (a file whose exact path you already hold) -- that is retrieval of a known target, not discovery. A sibling repo or submodule is NOT such an exemption by default: `codesearch {root: "<abs sibling/submodule path>", query, mode?}` searches it directly, its own persistent index cached at `<root>/.gm/gm.db`, so open-ended discovery in a sibling/submodule routes through `codesearch` with `root` set. `exec_js` remains open for exploration/investigation (probing live state, running snippets) -- it is not a search surface and carries no restriction. The line: known-path fetch = `Read` OK; discovery/search = verb only, always. The exhaustive exact-match sweep a `Grep`/`rg` gets reached for -- every definition AND every call site of one symbol, the "what calls Y" question above -- is `codesearch {query, mode: "literal"}`, or `mode: "regex"` for a pattern: it returns every match with `path` and `line`, no ranking, no top-k, reading the tree directly instead of the index. Check the response's `exhaustive` field -- `true` means the sweep is complete and the search is over. Full option list in entry.md's Inspection routing.
...
**A verification step that appears to require a human is first a build task, not an ask.** Asking a human to perform an action so you can observe the result -- press a button, turn a knob, listen to real audio, operate hardware the session has no other channel to -- is a missing channel, not an ambiguity, and the missing channel is itself a row: build or reuse an automated substitute that exercises the same code path before asking. For hardware I/O that means byte-level injection at the lowest available interface (raw MIDI note-on/note-off written straight to the device's rawmidi/ALSA node over the SSH channel already pulling logs, a captured byte sequence replayed from prior logs, the exact protocol messages the physical action would have produced). Fall back to asking only once the substitute has genuinely been attempted and either the defect depends on analog qualities injection cannot reproduce (audible quality, real timing jitter, electrical behaviour) or the human has said they want to verify by ear. A second identical physical-action ask in one session is the signal to build the harness instead of asking a third time.
...
**`prd-defer` is the one sanctioned exit for a row that is real, correctly scoped, and genuinely belongs to its own dedicated session -- never write `blockedBy` by hand.** The common case this covers: a session inherits an open row from a PRIOR session (`ready_wave`/`prd_pending>0` at entry), investigates it, and confirms it is a real, distinct piece of work outside this turn's scope (a flaky pre-existing test unrelated to this session's fix, a cross-repo change this session cannot drive) -- not something abandoned or too hard. Dispatch `prd-defer {"id":"<row-id>","reason":"<the concrete, specific reason this is genuinely out of reach this session, and what session/path would resolve it>"}`; it is gated exactly like `prd-add`'s own deferral check (`defer_marker_in_text`) -- bare "later"/"next session" phrasing is refused, the reason must name real substance. This is NOT a second "declare it externally blocked" exit for the row you're actually working on: it exists only for a row this session did not create and genuinely cannot resolve within its own scope. A row you can build past, per the paragraph above, still gets built past -- `prd-defer` is for the row that is real, someone else's, and out of THIS turn's scope, not a shortcut around doing the work.
