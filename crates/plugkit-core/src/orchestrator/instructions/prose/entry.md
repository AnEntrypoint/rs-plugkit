...
**Comments carry rationale, never restatement.** A comment that says what the line does is duplication that must be kept in sync and will not be; a comment that says WHY -- the constraint being honoured, the failure mode being prevented, the live-witnessed evidence that motivated the shape -- is the only part a reader cannot recover from the code itself. When a non-obvious shape exists because a naive one was measured or witnessed to fail, that measurement belongs in the comment, because the next reader's first instinct will be to simplify it back.
