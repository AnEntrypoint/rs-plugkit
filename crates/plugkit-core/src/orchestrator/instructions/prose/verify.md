...
**No test files, no exceptions.** A `deviation.synthetic-test-file` at VERIFY (new `*.test.*`/`*.spec.*`, a `test/`/`__tests__/` directory, a testing-framework import, or any standing root-level test file found in the diff) blocks `transition` exactly like an unwitnessed mutable -- remove it and replace its assertions with a live `exec_js`/`browser` witness, then check again. This is manual legwork, not framework legwork: the adversarial corner-case sweep above is how every class gets exercised, by running the real thing, never by writing a test case that runs it later.
...
**No comments, checked at VERIFY too, not only at EXECUTE.** A leading `//`, `///`, `/* */`, `#`, or JSDoc block anywhere in the diff -- inline, doc, or the paragraph-long rationale kind -- blocks `transition` exactly like an unwitnessed mutable: grep the diff for comment-opener tokens across every touched language before transitioning, remove what's found, and check again that the code still reads clearly by name and structure alone. A comment surviving into VERIFY means EXECUTE's own no-comments rule was skipped, not that this diff is the exception to it.
...
**Absence of edits is never grounds to skip a browser witness.** The check above only covers files this session actually edited -- a zero-edit turn trivially satisfies it, which is correct for that narrow question but wrong as a substitute for "does the app currently work?" The separate `app-loads-witnessed` gate closes this gap: on any project with a declared browser entrypoint (`.gm/browser-config.json` present), it requires a same-turn `browser` dispatch that reached the real page with zero console/page errors, regardless of whether any file changed this turn. A confirmation-only or audit-only pass asserting the app is healthy is itself a claim about runtime behavior, carrying the same L3 witness obligation as a code-change turn -- "the diff is clean, so nothing needs checking" is exactly the reasoning this gate blocks. `app-loads-witnessed` false -> dispatch `browser` against the real running app before `transition`, never argue the diff's emptiness as a substitute.
