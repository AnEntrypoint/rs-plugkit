...
fn default_graph() -> Graph {
    Graph {
        ...
        edges: vec![
            ...
            Edge { from: "VERIFY".into(), to: "CONSOLIDATE".into(), gates: vec!["residual-scan-fired".into(), "prd-all-closed".into(), "mutables-all-resolved".into(), "claim-audit-clean".into(), "submodules-clean".into(), "no-synthetic-test-files".into(), "no-admit-deferral-markers".into(), "no-secrets-in-diff".into(), "no-unchecked-panics-in-diff".into(), "no-hedge-language-in-diff".into(), "no-graphical-symbols-in-diff".into(), "idempotent-dispatch-replay-safe".into()] },
