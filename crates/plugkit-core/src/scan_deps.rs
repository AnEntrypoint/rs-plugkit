...
fn walk_package(
    ...
    for entry in crate::code_index::list_dir(dir) {
        ...
        if is_dir {
            ...
            // Each root-level package owns its scan. A workspace link can
            // resolve into a package with its own node_modules tree; descending
            // through it re-enters the dependency graph rather than this
            // package's source and can loop through workspace links.
