...
fn spawn_trajectory_ingest(project_dir: &str) {
    ...
    // Write to a temp file because long content via argv is unreliable on Windows .cmd shims.
    let ts = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis();
    let tmp = env::temp_dir().join(format!("gm-rslearn-trajectory-{}.txt", ts));
    if fs::write(&tmp, &summary).is_err() { return; }
    let tmp_str = tmp.to_string_lossy().to_string();
    spawn_detached_bun_rs_learn(
        &["add", "--file", &tmp_str, "--source", "trajectory/recent-commits", "--no-extract"],
        project_dir,
    );
    // Schedule cleanup 60s later by spawning a sleep+rm. Best-effort, not critical.
}
...
pub fn ingest_fast(content: &str, source: &str, project_dir: &str) {
    ...
    // Fallback: subprocess. Use file to avoid argv length limits.
    let ts = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis();
    let tmp = env::temp_dir().join(format!("gm-rslearn-ingest-{}.txt", ts));
    if fs::write(&tmp, content).is_err() { return; }
    let tmp_str = tmp.to_string_lossy().to_string();
    spawn_detached_bun_rs_learn(
        &["add", "--file", &tmp_str, "--source", source, "--no-extract"],
        project_dir,
    );
}
