...
Transition: residual-scan clear AND git clean AND CI green → dispatch `transition` to advance to UPDATE-DOCS. Anything else → dispatch `transition` back to PLAN or EXECUTE per the gap.
"#;
