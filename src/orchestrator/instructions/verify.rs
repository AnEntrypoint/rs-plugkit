...
- **The instant MicroLoRA adapter (the lora optimization)** refines the project's embedding space from trajectory quality. Dispatch `learn {verb:feedback, body:{embedding:<the bge embedding of this turn's task/query>, model:"recall", payload:{quality:<0–1>}}}`. The adapter is a rank-2 low-rank transform over the 384-dim bge vectors; high-quality trajectories nudge the embedding space so future recall lands closer to what actually helped. This is not LLM selection — it is the embedding model adapting to *this project's* semantic space, the core sona advance.
