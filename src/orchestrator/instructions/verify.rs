...
`git status --porcelain` is the `[worktree-clean]` witness — and it is a **separate Bash call you make before every possible push**, not an assumption you carry. Chaining `git commit && git push` skips the explicit witness; ccsniff `--git-discipline` flags it as `push-no-porcelain-witness` because the 20-event lookback shows no porcelain probe. The discipline is: `git status --porcelain` → read empty → `git push`, three calls visible in the transcript, every possible push preceded by its own probe. Non-empty bytes = unstaged residual; you stage-and-commit or revert before every possible push. A push from a dirty tree advances the chain on an unwitnessed slice — the bytes you didn't ship are the bytes that break the next session.
